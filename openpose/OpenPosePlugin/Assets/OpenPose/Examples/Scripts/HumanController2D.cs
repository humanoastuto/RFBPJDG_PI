...
namespace OpenPose.Example {
    ...
    public class HumanController2D : MonoBehaviour {
        ...
        public void DrawHuman(ref OPDatum datum, int bodyIndex, float scoreThres = 0){
            ...
            CheckFunnyScore(ref datum, bodyIndex);
        }

        public void CheckFunnyScore(ref OPDatum datum, int bodyIndex)
        {
            ...
            {
                double normalizedX = 964.378 - datum.poseKeypoints.Get(bodyIndex, 8, 0); //NormalizeTheMiddle
                double normalizedY = 167.014 - datum.poseKeypoints.Get(bodyIndex, 8, 1);
                double promBody = 0;
                for (int part = 0; part < poseJoints.Count; part++)
                {
                    promBody += datum.poseKeypoints.Get(bodyIndex, part, 0) + datum.poseKeypoints.Get(bodyIndex, part, 1);
                }
                promBody = promBody / 50;
                double percentage = ((100 * promBody) / 538.522942);
                Debug.Log("You got " + percentage + " % Accuracy");
            }
        }
        //538.522942
