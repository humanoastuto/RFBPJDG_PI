...
El método presentado es la normalización de los puntos dentro de la Point Cloud de tres formas distintas (presentadas por cada integrante del equipo), de las cuales se definirá la más adecuada. La normalización involucra ajustar la distancia entre los puntos para que tengan distancias proporcionalmente correctas a su forma original y permitan su comparación entre dos Point CLouds distintas.
 
 \subsection{Normalización a partir de la distancia entre dos puntos}
 
La normalización a partir de la distancia entre dos puntos elige dos puntos del modelo BODY\_25 de la figura \ref{open2}, el punto del cuello (Punto 1) y el punto de la cadera (Punto 8), ya que conforman la línea principal del cuerpo que une extremidades inferiores y superiores. Su distancia se iguala a una variable constante $ \alpha = 300$, seleccionada arbitrariamente para la normalización.
 
 
\subsection{Normalización a partir de un punto}

La normalización a partir de un punto busca un punto clave como base para normalizar el resto de los puntos, del modelo BODY\_25 de la figura \ref{open2} se especifico el centro del cuerpo, la cintura para la medición. 

Se mueve el punto clave (Punto 8) al centro de la imagen y se alinean los puntos de la persona de acuerdo a la distancia movida correspondiente como se ve en la figura \ref{overpose}. La debilidad de este método es que se requiere que las personas sean del mismo tamaño, por tanto, se debe normalizar la distancias de los puntos entre si.
...
Un ejemplo es desarrollado en el \ztitleref{anexocadera}, donde dadas 8 poses comparadas 


\subsection{Normalización del Cuadro de Point Cloud}

La normalización del cuadro de Point Cloud, se refiere a buscar los mínimos y los máximos de $(x,y)$ del conjunto de puntos en las Point Clouds del usuario y de la pose guardada en el mapa.
...
\begin{figure}[t!]
	...
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=7cm,height=5cm]{./Images/cuadropointcloud1.png}
